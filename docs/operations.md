...
`thread-mob-storage-vault` is centered on mobile workflows work in Go. The technical target is: Create a Go reference implementation for storage workflows, centered on visual model generation, layout fixtures, and stable geometry snapshots.
...
The score is deterministic. The current threshold is 157. Risk has a penalty of 4, latency has a penalty of 3, and weight contributes a bonus of 3.
